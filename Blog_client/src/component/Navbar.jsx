...
const Navbar = () => {
    ...
    useEffect(() => {
      if(token){
        ...
        axios.get(process.env.REACT_APP_SERVER_DOMAIN + "/user/new-notification", {
          headers : {
    ...
    const handleSearch = () => {
      setSearchVisibility(currval => !currval);
    }
    ...
    <>
      <nav className="navbar z-50">
        ...
        <div className="flex items-center gap-3 md:gap-6 ml-auto">
          <button className="md:hidden bg-grey w-12 h-12 rounded-full flex items-center justify-center" onClick={handleSearch}>
...

        {
          ...
          :
          <>
            <Link className="btn-dark py-2" to="/user/signin">
              Sign In
            </Link>

            <Link className="btn-light py-2 hidden md:block" to="/user/signup">
              Sign Up
            </Link>
          </>
        }
