...
import NotificationCommentField from "./ntfcn-comment-field";
...
const NotificationCard = ({data, index, notificationState}) => {
    ...
    let {type, seen, reply, createdAt, comment, replied_on_comment, user, user : {personal_Info : {fullname, username,profile_img}}, blog, blog: {_id, blog_Id, title}, _id: notification_id} = data;

    let {userAuth : {username: author_username, profile_img: author_profile_img, token}} = useContext(UserContext);
    ...
    const handleDelete = (comment_id, type, target) => {
        ...
        axios.post(process.env.REACT_APP_SERVER_DOMAIN + "/comment/delete-comment", {_id:comment_id}, {
            headers : {
            ...
            target.removeAttribbute("disabled");
    ...
    return (
        <>
            <div className={"p-6 border-b border-grey border-1-black " + (!seen ? "border-l-2" : "") }>
                <div className="flex gap-5 mb-3">
                    ...
                    <div className="w-full">
...

                        {
                            ...
                            :
                            <Link to={`/blog/${blog_Id}`} className="font-mediumtext-dark-grey hover:underline line-clamp-1">{`"${title}"`}</Link>
                        }
                    </div>
                </div>
