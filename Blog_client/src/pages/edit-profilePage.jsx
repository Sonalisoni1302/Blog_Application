...
const EditProfile = () => {
    ...
    const handleImgUpload = (e) => {
        ...
        if(updateProfileImg){
            ...
            // uploadeImage(updateProfileImg).then((url) => {
            //     
            //  if(url){
        //     axios.post(process.env.REACT_APP_SERVER_DOMAIN + "/user/update-profile-image", {url}, {
        //         headers : {
    ...
    const handleSubmit = (e) => {
        ...
        axios.post(process.env.REACT_APP_SERVER_DOMAIN + "/user/update-profile", {
            username, bio,
    ...
    useEffect(() => {
        if(token){
            axios.post(process.env.REACT_APP_SERVER_DOMAIN + "/user/get-profile", {username : userAuth.username}).then(({data}) => {
                setProfile(data);
                setLoading(false);
            })
        }
    }, [token, userAuth.username]) // eslint-disable-line react-hooks/exhaustive-deps
                        ...
                        <div className="flex flex-col lg:flex-row items-start py-10 gap-8 lg:gap-10">
                            ...
                            <div className="max-lg:center mb-5">
                                <label htmlFor="uploadImg" id="profileImgLable" className="relative block w-48 h-48 bg-grey rounded-full overflow-hidden">
                                    ...
                                    <img ref={profileImgEle} src={profile_img} alt="User profile" />
                                </label> 
                                <input type="file" id="uploading" accept=".jpe, .png, .jpg" hidden onChange={handleImagePreview}/>
                            ...
                            <div className="w-full">
                                ...
                                <p className="text-dark-grey -mt-3">Username will use to searchuser and will be visible to all users</p>

                                <textarea name="bio" maxLength={bioLimit} defaultValue={bio} className="input-box h-64 lg:h-40 resize-none leading-7 mt-5 pl-5" onChange={handleCharacterChange}></textarea>
                                ...
                                <div className="md:grid md:grid-cols-2 gap-x-6">
                                    {
                                        Object.keys(social_links).map((key, i) => {
                                            ...
                                            return <InputBox key={i} name={key} type="text" value={link} placeholder="http://" icon={"fi " + (key !== 'website' ? "fi-brands-" + key : "fi-rr-globe")}/>
                                        })
                                    }
                                </div>
